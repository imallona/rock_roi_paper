...
plot1<-rasterize(plot1, layers='Point', dpi=300)
...
plot2<-rasterize(plot2, layers='Point', dpi=300)
...
plot3<-rasterize(plot3, layers='Point', dpi=300)
...
# species is alredy random 

plot4<-ggplot(cd, aes(x = total, y = total_mito_percent)) +
  geom_point(aes(color=sce$species),size=0.1,alpha=0.3) + 
  scale_x_log10() + 
  scale_y_sqrt() +
...
ggsave(file=file.path(datadir,"qc.svg"), plot=plot4, width=5, height=2.7)
...
ggsave(file=file.path(datadir,"barnyard.svg"), plot=plot5, width=5.4, height=2.7)
...
plot7<-rasterize(plot7, layers='Point', dpi=300)

...
grep -P '^[ACTG]{9}AATG[ACTG]{9}CCAC' simulation_whitelists_with_prepends.txt >> output_simulation_whitelists_with_prepends.txt
grep -P '^[ACTG]{9,12}GTGA[ACTG]{9}GACA' simulation_whitelists_with_prepends.txt >> output_simulation_whitelists_with_prepends.txt

wc -l output_simulation_whitelists_with_prepends.txt
...
while read line
do
grep -P "^[ACTG]{0,3}${line}" output_simulation_whitelists_with_prepends.txt >> output_CLS1.txt
...
while read line
do
grep -P "^${line}" output_simulation_whitelists_with_prepends.txt >> output_CLS1.txt
done < ~/whitelist_96x3/BD_CLS1.txt

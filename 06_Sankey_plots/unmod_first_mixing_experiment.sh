...
ln -s ~/fastqs_seventh_scRNAseq/o307161_1-Unmodified_S4_R1_001.fastq.gz
...
zcat o307161_1-Unmodified_S4_R1_001.fastq.gz |awk 'NR%4==2' |grep -P '^[ACTG]{9}AATG[ACTG]{9}CCAC' > output.txt
...
echo "number of reads with TSO linkers" >> numbers_unmod.txt
wc -l output.txt >> numbers.txt

echo "number of reads with CL1" >> numbers_unmod.txt
wc -l output_CLS1.txt >> numbers.txt

echo "number of reads with CL2" >> numbers_unmod.txt
wc -l output_CLS2.txt >> numbers.txt

echo "number of reads with CL3" >> numbers_unmod.txt
wc -l output_CLS3.txt >> numbers.txt

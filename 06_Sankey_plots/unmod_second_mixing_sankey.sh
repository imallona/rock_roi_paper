...
cd /home/imallona/ebrunner_spectral/mixing/align_wta/mixing_unmod/
...
cd /home/imallona/ebrunner_spectral/mixing/align_tso/mixing_unmod/
...
ln -s /home/imallona/ebrunner_spectral/mixing/align_wta/mixing_unmod/Solo.out/Gene/filtered/barcodes.tsv valid_barcodes.tsv

samtools view -D CB:valid_barcodes.tsv -@ 20 Aligned.sortedByCoord.out.bam | cut -f1 | sort | uniq | wc -l

echo "multimappers TSO"
samtools view -D CB:valid_barcodes.tsv -@ 20 Aligned.sortedByCoord.out.bam | grep -vP '^@|NH:i:1\b' |  cut -f1 | sort | uniq | wc -l

echo "unique TSO"
samtools view -D CB:valid_barcodes.tsv -@ 20 Aligned.sortedByCoord.out.bam | grep -P '^@|NH:i:1\b'| cut -f1 | sort | uniq | wc -l

echo "unique in genes TSO"
samtools view -D CB:valid_barcodes.tsv -@ 20 Aligned.sortedByCoord.out.bam| grep -P '^@|NH:i:1\b' | grep -vP "gn:Z:-\t" | cut -f1 | sort | uniq | wc -l 

echo "unique not in genes TSO"
samtools view -D CB:valid_barcodes.tsv -@ 20 Aligned.sortedByCoord.out.bam| grep -P '^@|NH:i:1\b' | grep -P "gn:Z:-\t" | cut -f1 | sort | uniq | wc -l 

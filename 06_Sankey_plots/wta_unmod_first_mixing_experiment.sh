...
zcat o307161_1-Unmodified_S4_R1_001.fastq.gz |awk 'NR%4==2' |grep -P '^[ACTG]{9,12}GTGA[ACTG]{9}GACA' > output.txt
...
echo "number of reads with WTA linkers" >> wta_numbers_unmod.txt
wc -l output.txt >> wta_numbers_unmod.txt

echo "number of reads with CL1" >> wta_numbers_unmod.txt
wc -l output_CLS1.txt >> wta_numbers_unmod.txt

echo "number of reads with CL2" >> wta_numbers_unmod.txt
wc -l output_CLS2.txt >> wta_numbers_unmod.txt

echo "number of reads with CL3" >> wta_numbers_unmod.txt
wc -l output_CLS3.txt >> wta_numbers_unmod.txt

---
title: "Leukemia dataset, with sampletags"
...
```{r}
(dp <- '/home/imallona/ebrunner_spectral/leukemia/multimodal/')
(se_fn <- '/home/imallona/ebrunner_spectral/leukemia_sampletags/sampletags_summarizedexperiment.rds')
```
...
Graft the sampletags to the SCE's colData
...
# Detectability (TSO ontarget) per sampletag {.tabset .tabset-pills}
...
tryCatch({
    ...
    cat('## ', item, ' \n\n')
    print(VlnPlot(object = so,  features = item, group.by = 'sampletag')+
          theme(legend.position="right", aspect.ratio = 1))
    cat('\n\n')
    }, error = function(x) print(x))
}
...
colData(sce)$sampletag <- colData(sce)$sampletag

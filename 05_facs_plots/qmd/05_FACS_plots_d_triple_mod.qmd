---
title: "05_FACS_plots_d_triple_mod"
author: "Giulia Moro"
format: 
  html:
    toc: true
...
rdss <- dir(here("05_facs_plots/data/v2_triple_mod"), "*.fcs", 
...
titration<-titration[c(6,7,5,8,2,4,3,1)]

sampleNames(titration)<-c("pos_dt","pos_TSO","neg","unmod","3_gfp","3_tdtom","3_lgr5","3_dt")
pData(titration)$name<-c("pos_dt","pos_TSO","neg","unmod","3_gfp","3_tdtom","3_lgr5","3_dt")
...
ggcyto(titration, aes(x = FSC.A, y = SSC.A)) + 
  geom_hex(bins = 100) + 
  facet_wrap(~factor(name, levels = c("pos_dt","pos_TSO","neg","unmod","3_gfp","3_tdtom","3_lgra","3_dt"))) +
  geom_gate(beads) + 
...
ggcyto(filt_titration, aes(x = FSC.A, y = SSC.A)) + 
  geom_hex(bins = 100) + 
  facet_wrap(~factor(name, levels = c("pos_dt","pos_TSO","neg","unmod","3_gfp","3_tdtom","3_lgr5","3_dt"))) +
  geom_gate(beads) + 
  theme_bw() +
  facs_rock_roi_theme_multiple_panels()
...
ggcyto(filt_titration, aes(x = "APC-A", fill = name,color=name)) + 
  ...
  facet_wrap(~factor(name, levels = c("pos_dt","pos_TSO","neg","unmod","3_gfp","3_tdtom","3_lgr5","3_dt")),ncol=1,switch = "y")+
  theme_bw() +
...
ggplot(plottable,aes(x=factor(L1,levels=c("pos_dt","pos_TSO","neg","unmod","3_gfp","3_tdtom","3_lgr5","3_dt")),y=value))+

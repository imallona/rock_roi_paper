...
Add a 'count' assay summarizing the 'wta' assay

```{r}

counts(d) <- assay(d, "wta")
```
...
markers <- FindAllMarkers(object = so, max.cells.per.ident = 2000, only.pos = TRUE,
...
write.csv(x = markers, file = sprintf('mouse_batch_so_roi_markers_%s.csv', ID))
...
cors <- cor(as.matrix(GetAssayData(mini, 'RNA')[VariableFeatures(so),]), method = 'spearman')
...
column_ha = HeatmapAnnotation(roi_markers = t(as.matrix(GetAssayData(mini, 'RNA'))[top$gene,]),
...
## cors <- cor(as.matrix(GetAssayData(mini, 'data')[VariableFeatures(so),]), method = 'spearman')
## cors <- cor(as.matrix(GetAssayData(mini, 'data')), method = 'spearman')
...
```{r}
xlim <- c(0, ceiling(max(GetAssayData(pb, 'RNA'))))
...
for (item in c('rock_wta', 'rockroi_wta', 'roi_wta')) {
    plot(as.matrix(GetAssayData(pb, 'RNA'))[,item],
         as.matrix(GetAssayData(pb, 'RNA'))[,'unmod_wta'],
         pch = 19,
         cex = 0.5,
         col = ac('black', 0.1),
         xlim = xlim,
...
for (item in c('mixing_rock', 'mixing_rockroi', 'mixing_roi')) {

    y <- DGEList(GetAssayData(pb, slot = 'counts')[,c(item, 'mixing_unmod')], group=1:2)
    keep <- filterByExpr(y, design)
    ...
    tmp <- as.data.frame(as.matrix(GetAssayData(pb, 'RNA'))[,c(item, 'mixing_unmod')])
...
tmp <- data.frame(foo = as.matrix(GetAssayData(pb, 'RNA'))[,item],
                  bar = as.matrix(GetAssayData(pb, 'RNA'))[,'mixing_unmod'])
...
write.csv(x = markers, file = sprintf('human_batch_so_roi_markers_%s.csv', ID))
...
cors <- cor(as.matrix(GetAssayData(mini, 'RNA')[VariableFeatures(so),]), method = 'spearman')
## cors <- cor(as.matrix(GetAssayData(mini, 'RNA')), method = 'spearman')
...
for (item in c('rock_wta', 'rockroi_wta', 'roi_wta')) {
    plot(as.matrix(GetAssayData(pb, 'RNA'))[,item],
         as.matrix(GetAssayData(pb, 'RNA'))[,'mixing_unmod'],
         pch = 19,
         cex = 0.5,
         col = ac('black', 0.1),
         xlim = xlim,
...
for (item in c('rock_wta', 'rockroi_wta', 'roi_wta')) {
    ...
    hist(caution_noreps[[item]]$table$PValue, breaks = 100, main = sprintf('unadj pval %s vs mixing_unmod', item))
    ...
    print(ggplot(tmp, aes(y = get(item), x = mixing_unmod, label = gene)) +
        geom_point(color = idx, size = 0.5) +
        geom_text_repel(data = tmp[rownames(pb) %in% rownames(deg),], 
                        aes(y = get(item), x = mixing_unmod, label = gene)) +
        xlab('unmod wta lognorm counts') +
        ylab(sprintf('%s lognorm counts', item)) +
        theme_bw() +
        ggtitle(sprintf('%s vs mixing_unmod, human', item)))
    cat('\n\n')
}

...
df$cds_detection_score<-df$tso_capture_sum_cds+df$wta_capture_sum_cds
...
rownames(positive_cells)<-unique(df$sample)

for (i in unique(mouse$sample)){
  positive_cells[i,1]<-length(which(mouse[mouse$sample==i,]$cds_detection_score>0))/
    length(mouse[mouse$sample==i,]$cds_detection_score)*100
}

for (i in unique(mouse$sample)){
  positive_cells[i,2]<-length(which(human[human$sample==i,]$cds_detection_score>0))/
    length(human[human$sample==i,]$cds_detection_score)*100
}
...
ggplot(data=plottable,aes(y=value,x=variable,color=factor(sample,levels=c("mixing_unmod","mixing_unmod_roi",
                                                                          "mixing_rock","mixing_rockroi")))) +
  geom_point()+
  xlab(" ")+
  ylab("percent positive cells")+
...
ggplot(data=plottable,aes(y=value,x=factor(sample,levels=c("mixing_unmod","mixing_unmod_roi",
                                                           "mixing_rock","mixing_rockroi")),color=variable)) +
  geom_point()+
  xlab(" ")+
  ylab("percent positive cells")+
  theme_bw()+
  ...
  geom_text(
    aes(label=format(round(value,2),nsmall=2)), 
    position=position_dodge(width=0.5), 
    vjust=-0.5,show.legend=FALSE,size=4)+

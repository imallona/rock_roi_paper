...
plot1<-ggplot(cd, aes(x=factor(sample_id,levels=c("unmod","unmod_roi","rock","rockroi")), y=detected)) +
        geom_violin()+  
  theme_bw()+
  xlab("")+
  ylab("# genes WTA")+ 
...
ggsave(file=file.path(datadir,"genes.svg"), plot=plot1, width=2.7, height=2.7)


plot2<-ggplot(cd, aes(x=factor(sample_id,levels=c("unmod","unmod_roi","rock","rockroi")), y=total)) +
        geom_violin()+  
  theme_bw()+
  xlab("")+
...
ggsave(file=file.path(datadir,"transcripts.svg"), plot=plot2, width=2.75, height=2.75)

plot3<-ggplot(cd, aes(x=factor(sample_id,levels=c("unmod","unmod_roi","rock","rockroi")), y=total_mito_percent)) +
        geom_violin()+  
  theme_bw()+
  xlab("")+
...
ggsave(file=file.path(datadir,"mitochondria.svg"), plot=plot3, width=2.7, height=2.7)
...
cd$filt<-NA
cd[cd$sample_id=="unmod",]$filt<-3500
cd[cd$sample_id=="unmod_roi",]$filt<-3500
cd[cd$sample_id=="rock",]$filt<-2750
cd[cd$sample_id=="rockroi",]$filt<-3700
...
un<-as.data.frame(colData(sce_filt[,sce_filt$sample_id=="mixing_unmod"]))
un_roi<-as.data.frame(colData(sce_filt[,sce_filt$sample_id=="mixing_unmod_roi"]))
rock<-as.data.frame(colData(sce_filt[,sce_filt$sample_id=="mixing_rock"]))
rockroi<-as.data.frame(colData(sce_filt[,sce_filt$sample_id=="mixing_rockroi"]))

un<-un[,c("sample_id.barcode","species")]
un_roi<-un_roi[,c("sample_id.barcode","species")]
rock<-rock[,c("sample_id.barcode","species")]
rockroi<-rockroi[,c("sample_id.barcode","species")]

un$sample_id.barcode<-gsub("mixing_unmod.","",un$sample_id.barcode)
un_roi$sample_id.barcode<-gsub("mixing_unmod_roi.","",un_roi$sample_id.barcode)
rock$sample_id.barcode<-gsub("mixing_rock.","",rock$sample_id.barcode)
rockroi$sample_id.barcode<-gsub("mixing_rockroi.","",rockroi$sample_id.barcode)

rownames(un)<-un$sample_id.barcode
rownames(un_roi)<-un_roi$sample_id.barcode
rownames(rock)<-rock$sample_id.barcode
rownames(rockroi)<-rockroi$sample_id.barcode

write.csv(un,"species_unmod.csv")
write.csv(un_roi,"species_unmod_roi.csv")
write.csv(rock,"species_rock.csv")
write.csv(rockroi,"species_rockroi.csv")

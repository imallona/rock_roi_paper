...
fusion_db_sces_nd<-readRDS("/Users/gimoro/figure_making_paper/rock_roi_paper/08_cell_line_analysis/data/lines_and_patients.rds")$lines

dim(fusion_db_sces_nd) # dim: 21487 5434
dim(sces_nd) # dim: 21487  5434

rownames(assay(altExp(fusion_db_sces_nd)))
...
set.seed(1976)
nn.clusters <- clusterCells(fusion_db_sces_nd, use.dimred="PCA",
                            BLUSPARAM=NNGraphParam(k=70))
table(nn.clusters)
fusion_db_sces_nd$cluster <- nn.clusters

scater::plotUMAP(fusion_db_sces_nd, colour_by="cluster")

table(fusion_db_sces_nd$sample_id, fusion_db_sces_nd$cluster)

mm <- model.matrix(~cluster, 
                   data=as.data.frame(colData(fusion_db_sces_nd)))

lcounts <- logcounts(fusion_db_sces_nd)

col_anno <- colData(fusion_db_sces_nd)[,c("cluster")] %>% as.data.frame

scater::plotUMAP(fusion_db_sces_nd, colour_by = "cluster")


plot3<-scater::plotUMAP(fusion_db_sces_nd, colour_by = "cluster",point_alpha=1,point_size=0.1)+
...
fusion_db_sces_nd$annotation[fusion_db_sces_nd$cluster == 1] <- "Loucy"
fusion_db_sces_nd$annotation[fusion_db_sces_nd$cluster == 2] <- "SUP-B15"
fusion_db_sces_nd$annotation[fusion_db_sces_nd$cluster == 3] <- "K562"

...
datadir<-"/Users/gimoro/figure_making_paper/rock_roi_paper/09_hela_analysis/data"

counts <- readMM(file.path(datadir,'matrix.mtx'))
genes <- read.table(file.path(datadir,'features.tsv'), header = FALSE)
cell_ids <-readLines(file.path(datadir,'barcodes.tsv'))
...
dim(sce)
sces <- sce[,sce$total>10000 & sce$subsets_mouse_mito_percent>3 & sce$subsets_mouse_mito_percent<25]

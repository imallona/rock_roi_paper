...
sce<-readRDS(file.path(datadir,"unmod_1.sce"))
...
plot2<-ggplot(cd, aes(x = total, 
  ...
  geom_vline(xintercept=c(25000), colour="orange") +
   theme_bw()+
  theme(
    axis.text = element_text(size = 15),
    axis.title=element_text(size=20,face="bold"),
...
dim(sce)
sces <- sce[,sce$total>25000 & sce$subsets_human_mito_percent>3 & sce$subsets_human_mito_percent<25]

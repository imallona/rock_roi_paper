...
# plotUMAP(sce, colour_by = "cluster", shape_by = "sample_id") + 
#   facet_wrap(~shape_by) +
#   scale_colour_manual()
...
myscale <- c(color("muted")(9), gray="lightgray", black="black")
names(myscale) <- as.character(1:11)
...
table(rs <- rowSums(lcounts)>20 & rowData(sce)$gene_type == "mouse_gene" )
...
plotUMAP(sce, colour_by = "ENSMUSG00000025934.15__Gsta3")
...
plotExpression(sce, x = "cluster", features = c("ENSMUSG00000033740.17__St18",
               ...
               colour_by = "cluster") +
  scale_color_manual(values = myscale)
# plotUMAP(sce, colour_by = "ENSMUSG00000026193.15__Fn1")
...
save_pheatmap_pdf(ph, "tuft_clustering.pdf", width = 12, height = 14)

# rownames(altExp(sce))
# sort(rowSums(assay(altExp(sce), 1)), decreasing = TRUE)
...
ggsave("tuft_markers_by_cluster.pdf", p, height = 12, width = 12)

...
gs <- rownames(tt)[tt$F > 400 & ents < 1]
...
exprs_nd <- assay(altExp(sces),1)

# exprs_nd <- assay(sces, "tso_off_and_ontarget_unique")
# keep <- rownames(exprs_nd)[rownames(exprs_nd) %in% rownames(altExp(sces))]
...
df <- data.frame(mouse = (sces$origin == "mouse")+0,
                 human = (sces$origin == "human")+0,
                 sample = sces$sample_id,
